    ...
    <section className="max-w-6xl mx-auto px-6 py-20" id="certifications">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {certifications.map((cert) => (
          <div
            key={cert.id}
            onClick={() => cert.certification_url && setSelected(cert)}
            className={`glass dark:glass p-6 rounded-2xl border border-primary/10 hover:border-primary/30 transition-all ${cert.certification_url ? 'cursor-pointer hover:scale-[1.02]' : ''}`}
          >
      ...
      {selected && (
        ...
        >
          <div className="relative max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <img
              src={selected.certification_url!}
              alt={selected.name}
              className="w-full rounded-xl shadow-2xl"
            />
            <p className="text-center text-white mt-4 font-semibold">{selected.name}</p>
          </div>
        </div>
      )}
    </section>
